<?php
/**
 * @file
 */
if ( !defined( 'MEDIAWIKI' ) ) {
	die();
}

/**
 * Main User Interface HTML template for Special:Donate
 *
 * @ingroup Templates
 */
class SpecialDonateUITemplate extends QuickTemplate {
	public function execute() {
		$encryptedValue = '-----BEGIN PKCS7----------END PKCS7-----';
?>
<table>
	<tbody>
		<tr>
			<td>
			<form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_blank">
				<input type="hidden" name="cmd" value="_s-xclick" />
				<input type="hidden" name="encrypted" value="<?php echo $encryptedValue ?>" />
				<input type="image" src="https://www.paypalobjects.com/en_US/i/btn/btn_donateCC_LG.gif" border="0" name="submit" alt="<?php echo wfMessage( 'donate-paypal-alt' )->plain() ?>" />
				<img alt="" border="0" src="https://www.paypalobjects.com/en_US/i/scr/pixel.gif" width="1" height="1" />
			</form>
			</td>
			<td>
				<div id="donate-callout"><big><b><?php echo wfMessage( 'donate-callout-details' )->escaped() ?></b></big><br /><?php echo wfMessage( 'donate-callout-details' )->parse() ?></div>
			</td>
		</tr>
	</tbody>
</table>
<?php
	}
}